$(document).ready(function () {
    ...
    d.style("visibility", "hidden"),
        d.select(".searchbar").on("change", function () {
            ...
            null == t || 0 === t.length ? x.select("#relation-container").select(".current").selectAll(".relation")
                ...
                .select(".current").selectAll(".relation").transition().duration(500).style("opacity", function (t) {
                    return t.name === e[0] ? 1 : 0
                }) : 2 === e.length && "p" === e[0] && x.select("#relation-container").select(".current")
                .selectAll(".relation").transition().duration(500).style("opacity", function (t) {
    ...
    function E(t, e) {
        d.style("visibility", "visible"),
            d3.select(".kg-graph").select(".predicate-filter").style("visibility", "visible"),
            n = 1,
            M = [],
            x.select("#node-container").selectAll(".node").data(M).exit().remove(),
            x.select("#concept-container").remove(),
            x.select("#spiral-container").remove(),
            N(M, {
    ...
    function I() {
        ...
        !function (t, n) {
            ...
            l.reduce(function (t, e, n) {
                ...
                $.each(c, function (t, e) {
                    rr = Math.random() / 5 + .15,
                        k = (cur + Math.round(rr * (sibs.length - 1))) % sibs.length,
                        cur = k,
                        sibs[cur].angle = e,
                        r[t] = sibs[cur],
                        sibs.splice(cur, 1)
        ...
        let e = x.select("#relation-container").select(".current").selectAll(".relation").data(function (t) {
            ...
            ,
            n = (e.append("line").attr("class", "line").attr("x1", "0").attr("y1", "0").attr("x2", "0").attr("y2", "0")
                .style("stroke", function (t) {
                    return B(t)
                }).style("stroke-width", 2).on("mouseover", function (t) {
                    g || (u = !1,
                    t.context && (c.select(".title").text("潜在关系解释"),
                        c.select(".des").text(t.context),
                        c.select(".result-img a").style("background-image", "url('" + t.image + "')"),
                        c.transition().duration(200).style("opacity", 1)))
                }).on("mouseout", function (t) {
                    g || (u = !0,
                        c.transition().duration(200).style("opacity", 0))
                }).on("mousemove", function (t) {
                    g || (1e3 < m ? (d3.event.pageX > m / 2 + 50 ? c.style("left", d3.event.pageX - 20 - 520 + "px") :
                        d3.event.pageX < m / 2 - 50 ? c.style("left", d3.event.pageX + 10 + "px") :
                            c.style("left", d3.event.pageX - 250 + "px"),
                        d3.event.pageY > h / 2 ? c.style("top", d3.event.pageY - 150 + "px") :
                            c.style("top", d3.event.pageY + 30 + "px")) : c.style("position", "fixed")
                        .style("width", "400").style("height", "100").style("left", "calc(50% - 200px)")
                        .style("top", "calc(50% - 50px)").style("display", "block"))
                }),
                e.append("g").attr("class", "objectAndText"))
            , r = (n.append("defs").attr("id", "imgdefs").append("pattern").attr("id", function (t, e) {
                return t._graphid
            }).attr("height", 1).attr("width", 1).attr("x", "0").attr("y", "0").append("image").attr("x", 0)
                .attr("y", 0).attr("class", "objectImage").attr("xlink:href", function (t) {
                    return t.image
                ...
                n.append("circle").attr("class", "object").attr("cursor", "pointer").on("mouseover", function (t) {
                    g || (u = !1,
                        c.select(".title").text(t.name),
                        c.select(".des").text(t.desc || "暂无简介"),
                        c.select(".result-img a").style("background-image", "url('" + t.image + "')"),
                        c.transition().duration(200).style("opacity", 1))
                }).on("mouseout", function (t) {
                    g || (u = !0,
                    ...
                    g || (1e3 < m ? (d3.event.pageX > m / 2 + 50 ? c.style("left", d3.event.pageX - 20 - 520 + "px") :
                        ...
                        d3.event.pageY > h / 2 ? c.style("top", d3.event.pageY - 150 + "px") :
                            c.style("top", d3.event.pageY + 30 + "px")) : c.style("position", "fixed")
                        .style("width", "400px").style("height", "100px").style("left", "calc(50% - 200px)")
                        .style("top", "calc(50% - 50px)").style("display", "block"))
                                ...
                                e === n && (n += .1),
                                    ...
                                    $(".dummy").velocity({
                                        ...
                                        complete: function (t) {
                                            ...
                                            N(M, r),
                                                H(M.length - 1, function (t, e) {
                                                    y = !0,
                                                    e === M.length - 1 && (u = !0,
                                                        t.entity ? q(t.entity, function (t, e) {
                                                            y = !1,
                                                                M[M.length - 1].relations = e,
                                                                g = !1,
                                                                I(),
                                                                W(100)
                ...
                n.append("text").attr("dy", "0.5em").attr("dx", "0").attr("class", "objectText")
                    .attr("visibility", "hidden"));
    ...
    function _(t) {
        ...
        }).attr("height", 1).attr("width", 1).attr("x", "0").attr("y", "0").append("image").attr("x", 0).attr("y", 0)
            .attr("height", 50).attr("width", 50).attr("preserveAspectRatio", "xMidYMid slice")
            .attr("xlink:href", function (t) {
                return t.image
            }),
            t.append("circle").attr("r", 25).attr("class", "subject").style("stroke", "#1d1d1d")
                .style("stroke-width", 6).attr("fill", function (t) {
    ...
    function O(t, e) {
        ...
        return h / 10 < (r *= r) && (r = h / 10),
            $.each(t.sibs, function (t, e) {
                diff2 = e.distance - f,
                    distance2 = h / 3.8 + h / 40 * diff2,
                distance2 > n / 2 && (r2 = Math.sqrt(-2 * distance2 * n * Math.cos(e.angle) +
                    distance2 * distance2 + n * n),
                    r2 = r2 * n / (n + distance2),
                r2 < r && 0 !== e.angle && (r = r2))
            }),
        r < 5 && (r = 5),
        t.fixedDistance && (r = 25),
            r
    }
        ...
        if (null != t && null != t.relations) {
            ...
            i.exit().remove(),
                i.enter().append("div").attr("class", "item").merge(i).style("background-color", function (t) {
                    ...
                    g || (1e3 <= m ? c.style("left", d3.event.pageX - 40 - 530 + "px")
                        .style("top", d3.event.pageY - 80 + "px") : c.style("position", "fixed").style("width", "400px")
                        .style("height", "100px").style("left", "calc(50% - 200px)").style("top", "calc(50% - 50px)")
                        .style("display", "block"),
                        c.select(".title").text(t.name),
                        c.select(".des").text(t.desc || "暂无简介"),
                        c.select(".result-img a").style("background-image", "url('" + t.image + "')"),
    ...
    function N(t, e) {
        t.push(e),
            x.select("#relation-container").selectAll(".relations").data(t).classed("current", !1).enter().append("g")
                .attr("class", "relations current").attr("transform", function (t) {
                return "translate(" + t.x + "," + t.y + ")"
            }),
            I(),
            _(x.select("#node-container").selectAll(".node").data(t).enter().append("g").attr("class", "node")
    ...
    function q(t, u) {
        $.get("/cndbpedia/kggraphData?entity=" + t, function (t) {
            ...
            $("#predicate-select").dropdown("clear"),
                $("#predicate-select").dropdown("setup menu", {
                    values: [{
                        value: "-1",
                        text: "全部",
                        name: "全部"
                ...
                $("#predicate-select").dropdown("setting", {
                    onChange: function (e) {
                        -1 === e ? x.select("#relation-container").select(".current").selectAll(".relation")
                            .transition().duration(500).style("opacity", 1) : x.select("#relation-container")
                            .select(".current").selectAll(".relation").transition().duration(500)
        ...
        (u || t) && (d3.selectAll(".current .objectAndText").attr("transform", function (t, e) {
            ...
            d3.selectAll(".current .objectText").attr("visibility", function (t, e) {
                ...
                let n = this.getBBox()
                    ...
                    , c = 0
                    , o = S(t, p);
                return 7 * s / 4 < o || o <= s / 4 ? (l = r / 2 + i + 15,
                    c = 0) : s / 4 < o && o <= 3 * s / 4 ? (l = 0,
                    c = i + 15 + a / 2) : 3 * s / 4 < o && o <= 5 * s / 4 ? (l = -(r / 2 + i + 15),
                    c = 0) : (l = 0,
                    c = -(i + 15 + a / 2)),
                "translate(" + l + "," + c + ")"
            })),
        t || (u && o <= (p += s / 3600) && (p -= o),
            y ? w.attr("visibility", "visible").attr("transform", function (t) {
                return "rotate(" + 3600 * p + ")"
            }) : w.attr("visibility", "hidden"),
            setTimeout(F, 10))
    }
    ...
    q(X, function (g, y) {
        t ? E(g, y) : $.get("/cndbpedia/kggraphConcepts?entity=" + X, function (t) {
            ...
            let a = d3.min([m - 40, h - 40]) / 2
                ...
                ,
                o = x.select("#spiral-container").append("path").datum(i).attr("id", "spiral").attr("d", c)
                    .style("fill", "none").style("stroke", "gray")
                , s = o.node().getTotalLength();
            o.attr("stroke-dasharray", s + " " + s).attr("stroke-dashoffset", s).transition().duration(1e4)
            ...
            f.attr("transform", function (t, e) {
                ...
                f.append("circle").attr("r", 6).attr("fill", "darkgray"),

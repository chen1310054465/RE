...
class model:
    def __init__(self, data_loader, et=False, activation=tf.nn.relu, is_training=True):
        ...
        data_loader.data_require = self.data_require
    ...
    def _encoder(self):
        if FLAGS.en == "cnn" or FLAGS.en == "pcnn":
            ...
            self.encoder = encoder.resnet(self.wp_embedding, filters=[FLAGS.f1, FLAGS.f2], ib_num=FLAGS.ib_num,
                                          mask=self.mask, activation=self.activation, keep_prob=self.keep_prob)
            ...
            if en == "rnn" or en == "birnn":
                ...
                if ens[0] == 'resnet':
                    self.encoder = encoder.resnet(self.wp_embedding, filters=[FLAGS.f1, FLAGS.f2], length=self.length,
                                                  cell_name=cell_name, bidirectional='bi' in en, ib_num=FLAGS.ib_num,
                                                  mask=self.mask, activation=self.activation, keep_prob=self.keep_prob)
                else:
